 ...
 * طبقة البيانات المتكاملة بين الكود الموجود (camelCase + localStorage)
 * وقاعدة بيانات Supabase (snake_case + UUID)
 * 
 * المميزات:
 * - ترجمة تلقائية بين camelCase و snake_case
 * - مصادقة عبر Supabase Auth
 * - مزامنة Offline مع localStorage
 * - التعامل مع UUID
 */

// ============================================================
// 1. خريطة تحويل أسماء الحقول (camelCase <-> snake_case)
// ============================================================
const FIELD_MAPPINGS = {
    ...
    // جدول الأرقام الوظيفية المسموحة (allowed_employee_ids)
    allowedEmployeeIds: {
        id: 'id',
        empId: 'emp_id',
        name: 'name',
    ...
    // طلبات الصيانة والمعايرة (maintenance_requests)
    maintenanceRequests: {
        id: 'id',
        deviceId: 'device_id',
        serialNumber: 'serial_number',
...
// ============================================================
// خريطة تحويل اسم الجدول (snake_case DB) → مفتاح FIELD_MAPPINGS
// ============================================================
const TABLE_NAME_TO_MAPPING = {
    ...
    'worker_leave_requests': 'workerLeaveRequests',
    'salary_raise_requests': 'salaryRaiseRequests',
...
function toSnakeCase(tableName, data) {
    ...
    // إضافة أي حقول إضافية غير موجودة في الخريطة
...
const SupabaseAuth = {
    ...
    async signIn(email, password) {
        try {
            ...
            // حفظ الجلسة في localStorage (للتوافق مع الكود الحالي)
            localStorage.setItem('sajco_session', JSON.stringify(user));
            
            return { success: true, user: user };
        } catch (error) {
            // في حال فشل الاتصال بـ Supabase، نستخدم localStorage كبديل
    ...
    /**
     * بديل تسجيل الدخول عبر localStorage (حالة عدم الاتصال)
     */
    fallbackSignIn(email, password) {
        const user = db.users.find(u => u.email === email && u.password === password);
        if (!user) {
            return { success: false, error: 'بيانات الدخول خاطئة' };
        }
        if (user.status === 'pending') {
            return { success: false, error: 'حسابك قيد المراجعة' };
        }
        localStorage.setItem('sajco_session', JSON.stringify(user));
        return { success: true, user: user };
    },
    ...
    async signUp(email, password, userData) {
        try {
            ...
            if (!dbResult.success) {
                // Rollback: حذف حساب Auth إذا فشل إدراج المستخدم
                await supabaseClient.admin.deleteUser(authData.user.id);
                throw new Error(dbResult.error);
            }

            // 3. إضافة إلى localStorage للتوافق
...
const SupabaseDataLayer = {
    ...
    async delete(tableName, id) {
        try {
            ...
            console.error(`خطأ في حذف ${tableName}:`, error.message);
    ...
    async query(tableName, filters = {}, options = {}) {
        try {
            ...
            console.error(`خطأ في استعلام ${tableName}:`, error.message);
            return { success: false, error: error.message, data: [] };
        }
    },

    // ============================================================
    // 5. دوال خاصة بكل جدول (تبسيط الاستخدام)
    // ============================================================
    ...
    async getUserById(id) {
        ...
        return user ? { success: true, data: user } : { success: false, error: 'المستخدم غير موجود' };
    },
    ...
    async getAllowedEmployeeIds() {
        try {
            const { data, error } = await supabaseClient
                ...
                .select('emp_id, name');
            if (error) throw error;
            // إرجاع مصفوفة كائنات {empId, name} للتوافق مع النظام الجديد
    ...
    /**
     * تحويل ID قديم إلى UUID مع الحفاظ على التعيين
     */
    _convertId(oldId) {
        if (!oldId) return this._generateUUID();
        // إذا كان UUID بالفعل، ارجعه كما هو
        if (oldId.includes('-') && oldId.length === 36) return oldId;
        // إنشاء UUID محدد بناءً على الـ ID القديم (للحفاظ على الاتساق)
    ...
    /**
     * نقل جميع البيانات من localStorage إلى Supabase
     */
    async migrateAllToSupabase() {
        ...
        // 2. المستخدمون (بدون كلمة المرور - ستُدار عبر Auth)
        for (const user of db.users) {
            ...
            try {
                const snakeData = toSnakeCase('users', {
                    ...
                    password: undefined, // لا ننقل كلمة المرور
    ...
    fallbackGetById(tableName, id) {
        ...
        const localKey = tableMap[tableName];
        if (!localKey) return { success: false, error: 'جدول غير معروف' };
        const item = db[localKey]?.find(item => item.id === id);
        return item ? { success: true, data: item } : { success: false, error: 'العنصر غير موجود' };
    },
    ...
    fallbackUpdate(tableName, id, updates) {
        ...
        if (index === -1 || index === undefined) return { success: false, error: 'العنصر غير موجود' };
...
/**
 * تسجيل الدخول المتكامل (يحاول Supabase أولاً، ثم localStorage)
 */
...
// ============================================================
// 9. مزامنة البيانات عند الاتصال
// ============================================================

/**
 * مزامنة جميع البيانات المحلية مع Supabase عند توفر الاتصال
 */
async function syncLocalToSupabase() {
    if (typeof supabaseClient === 'undefined' || !supabaseClient) {
        console.warn('⚠️ Supabase غير متصل، تخطي المزامنة');
    ...
    for (const custody of db.custodies) {
        try {
            // تجاهل العهدة القديمة التي لا تحتوي على UUID متوافق
            const { id, userId, assignedBy, branchId, ...custodyData } = custody;
            const snakeData = toSnakeCase('custodies', custodyData);
            
            // ملاحظة: سيتم تجاهل سجلات العهدة إذا لم تكن مرتبطة بـ UUID مستخدم صحيح
            // يفضل المزامنة بعد تسجيل الدخول عبر Supabase Auth
    ...
    console.log(`✅ تمت مزامنة ${results.success} سجل بنجاح، فشل ${results.failed}`);
    return results;
}

// ============================================================
// 10. تصدير للاستخدام (CommonJS + ES Module)
// ============================================================

// تصدير بصيغة CommonJS (للاستخدام في Node.js)
...
// تصدير بصيغة ES Module (للاستخدام مع Vite, Webpack, TypeScript)
// استخدم: import { SupabaseDataLayer, SupabaseAuth } from './supabase-data-layer.js'
if (typeof exports !== 'undefined') {
    // Named exports للاستخدام مع ES Modules
    // يمكنك استخدام:
